...
{
    // if p is a dummy node or the node preceding the dummy node, do not swap
    if(p->prev == nullptr || p->next == nullptr || p->next->next == nullptr){
        return;
    }

    // initialize beforep pointer and afterp pointer
    Node * beforep = p->prev;
    Node * afterp = p->next;

    // swap p, afterp, and adjacent node pointers
    p->next = afterp->next;
    afterp->next = afterp;
